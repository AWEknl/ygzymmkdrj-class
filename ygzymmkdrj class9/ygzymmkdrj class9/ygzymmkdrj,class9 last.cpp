...
int main()
{
	...
	switch (q) {
	case 7:
		printf("AWEkn1 (2022.6.7 to 2024.6.8)\n");
		printf("请输入:");
		scanf("%d", &q);
		break;
	case 8:
	...
	case 9:
		...
		case 2:
			{
				asd();
				printf("+       + class9 2024.6.8 16:48                                               +\n");
				printf("---------因为class8出现了乱码，所以使用class7改造           +\n");
				printf("+       +  本次更新:优化代码， 并且使用visual studio2008编译                     +\n");
				printf("--------- （可惜这不是最后更新。）                                      +\n");
				printf("+       +   当前版本:class9                        +\n");
				abab();
				acac();
	...
	{
		asd();
		printf("+退出7  +class8-9是一个大的跨越，时隔两年，终于更新一次，                   +\n");
		printf("---------两年后，当我收拾自己的文档时，发现我还有个被遗弃的项目“ygzymmkdrj”                           +\n");
		printf("+help 8 +aqua  Plan:                       +\n");
		printf("---------重写代码，对代码进行全部的优化。                         +\n");
		printf("+       +    +（当然这还不一定，也有可能不会再继续了）\n");
		printf("---------AWEknl                           +\n");
		printf("+       +      -2024.6.2 17:35                            +\n");
		printf("---------                     +\n");
		acac();
		abcd();
		qustion();
